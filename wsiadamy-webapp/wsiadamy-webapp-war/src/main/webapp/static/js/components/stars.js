$(function() {
  $(".stars.starsEditable").jRating({
	  step: true,
	  length: 5,
    rateMax: 5,
	  canRateAgain: true,
    nbRates: 9999,
    bigStarsPath: $('body').data('basepath') + '/static/img/jrating/stars.png',
    callback: function(rate, layer) {
      if(layer.data('bindvalue'))
        $('#' + layer.data('bindvalue')).val(rate);
    }
	});
  
  /*$('.stars.starsEditable').each(function() {
    var o = $(this);
    var i = o.data('range');
    ...
    if(o.find('a').length) {
      width = o.find('a:first').width() * o.find('a').length;
    }
    
    o.css('width', width);
  });
  
  $('.stars.starsEditable a').hover(function() {
